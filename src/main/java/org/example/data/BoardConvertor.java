...
public class BoardConvertor implements Convertor<Board, BoardPOJO> {
    ...
    public Case createCase(Character symbol, int position) {
        switch (symbol) {
            ...
            case 'N' -> {
                Case caseBonusEffect = new Case(position);
                caseBonusEffect.setCaseEffect(new BonusEffect());
                return caseBonusEffect;
            }
            case 'P' ->{
                Case casePrisonEffect = new Case(position);
                casePrisonEffect.setCaseEffect(new PrisonEffect());
                return casePrisonEffect;
            }
            default -> {
    ...
    public Character toSymbol(Case c) {
        switch (c.getCaseEffect()) {
            ...
            case BonusEffect _ -> { return 'N'; }
            case PrisonEffect _ -> { return 'P'; }
