...
class QuestionJDBCDAOTest {
    ...
    void get() {
        try {
            ...
            Optional<QuestionPOJO> question = dao.get(0);
            assertTrue(question.isPresent());
            assertEquals("What?", question.get().getQuestion());
            assertEquals("yes", question.get().getAnswer());
            Optional<QuestionPOJO> question2 = dao.get(1);
    ...
    void getAll() {
        try {
            QuestionJDBCDAO dao = new QuestionJDBCDAO();
            List<QuestionPOJO> questions = dao.getAll();
    ...
    void create() {
        try {
        QuestionJDBCDAO dao = new QuestionJDBCDAO();
        QuestionPOJO dto = new QuestionPOJO( "Here?", "no");
        int newId = dao.create(dto);
        assertEquals(1, newId);
        Optional<QuestionPOJO> question = dao.get(newId);
        assertTrue(question.isPresent());
        assertEquals("Here?", question.get().getQuestion());
        assertEquals("no", question.get().getAnswer());
        }
        catch (Exception e) {
            fail(e.getMessage());
        }
    ...
    void update() {
        try {
            QuestionJDBCDAO dao = new QuestionJDBCDAO();
            QuestionPOJO dto = new QuestionPOJO("What?", "no");
            dao.update(dto);
            Optional<QuestionPOJO> question = dao.get(0);
            assertTrue(question.isPresent());
            assertEquals("What?", question.get().getQuestion());
